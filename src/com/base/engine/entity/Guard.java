...
import com.base.engine.graphics.Texture;
...
public class Guard {
	...
	public Guard(Transform transform) {
		if (rand == null) {
			rand = new Random();
		}
		...
		if (mesh == null) {
			...
			Vertex[] verts = new Vertex[] { new Vertex(new Vector3f(-sizeX, 0, 0), new Vector2f(texMaxX, texMaxY)), new Vertex(new Vector3f(-sizeX, sizeY, 0), new Vector2f(texMaxX, texMinY)), new Vertex(new Vector3f(sizeX, sizeY, 0), new Vector2f(texMinX, texMinY)), new Vertex(new Vector3f(sizeX, 0, 0), new Vector2f(texMinX, texMaxY)) };
	...
	public void update() {
		...
		if (!dead) {
			...
			Vector2f playerDirection = transform.getPosition().sub(player.getCamera().getPos().add(new Vector3f(player.getSize().getX(), 0, player.getSize().getY()).mul(0.5f))).getXZ().normalized();
			...
			if (state == STATE_IDLE) {
				...
				if (timeDecimals >= 0.5) {
					...
					if (canLook) {
						...
						if (playerIntersect != null && (nearestIntersect == null || nearestIntersect.sub(lineStart).length() > playerIntersect.sub(lineStart).length())) {
