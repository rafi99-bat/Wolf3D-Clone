...
import com.base.engine.entity.Door;
import com.base.engine.entity.Guard;
import com.base.engine.entity.Medkit;
import com.base.engine.entity.Player;
import com.base.engine.graphics.Bitmap;
import com.base.engine.graphics.Material;
import com.base.engine.graphics.Mesh;
import com.base.engine.graphics.Vertex;
import com.base.engine.input.Input;
import com.base.engine.maths.Vector2f;
import com.base.engine.maths.Vector3f;
import com.base.engine.utils.AudioUtil;
import com.base.engine.utils.PhysicsUtil;
...
public class Level {
	...
	public void input() {
		if (Input.getMouseDown(0))
			for (Guard guard : guards)
				if (Math.abs(guard.getTransform().getPosition().sub(player.getCamera().getPos()).length()) < 1.5f)
					guard.damage((int) (30f + rand.nextFloat() * 50f));
	...
	private void sortMonsters(int low, int high) {
		...
		Guard pivot = guards.get(low + (high - low) / 2);
		float pivotDistance = pivot.getTransform().getPosition().sub(Transform.getCamera().getPos()).length();

		while (i <= j) {
			while (guards.get(i).getTransform().getPosition().sub(Transform.getCamera().getPos()).length() > pivotDistance) {
				i++;
			}
			while (guards.get(j).getTransform().getPosition().sub(Transform.getCamera().getPos()).length() < pivotDistance) {
				j--;
			}

			if (i <= j) {
				Guard temp = guards.get(i);

				guards.set(i, guards.get(j));
				guards.set(j, temp);
	...
	public Vector2f checkIntersections(Vector2f lineStart, Vector2f lineEnd, boolean hurtMonsters) {
		...
		for (int i = 0; i < collisionPosStart.size(); i++) {
			...
			if (collision != null && (nearestIntersect == null || nearestIntersect.sub(lineStart).length() > collision.sub(lineStart).length()))
		...
		if (hurtMonsters) {
			...
			for (Guard guard : guards) {
				Vector2f collision = PhysicsUtil.lineIntersectRect(lineStart, lineEnd, guard.getTransform().getPosition().getXZ(), guard.getSize());

				if (collision != null && (monsterIntersect == null || monsterIntersect.sub(lineStart).length() > collision.sub(lineStart).length())) {
					monsterIntersect = collision;
					nearestGuard = guard;
				}
			}

			if (monsterIntersect != null && (nearestIntersect == null || nearestIntersect.sub(lineStart).length() > monsterIntersect.sub(lineStart).length()))
				nearestGuard.damage(player.getDamage());
		}
			...
			for (int j = 1; j < level.getHeight() - 1; j++) {
				if ((level.getPixel(i, j) & 0xFFFFFF) != 0) // If it isn't a black (wall) pixel
				{
					if ((level.getPixel(i, j) & 0x0000FF) == 16) {
						...
						guards.add(new Guard(new Transform(new Vector3f((i + 0.5f) * SPOT_WIDTH, 0, (j + 0.5f) * SPOT_LENGTH))));
