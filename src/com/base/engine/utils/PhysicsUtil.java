...
//	}
	...
	public static Vector2f rectCollide(Vector2f oldPos, Vector2f newPos, Vector2f size1, Vector2f pos2, Vector2f size2) {
		...
		if (!(newPos.getX() + size1.getX() < pos2.getX() || newPos.getX() - size1.getX() > pos2.getX() + (size2.getX() * size2.getX()) || oldPos.getY() + size1.getY() < pos2.getY() || oldPos.getY() - size1.getY() > pos2.getY() + (size2.getY() * size2.getY())))
			result.setX(0);

		if (!(oldPos.getX() + size1.getX() < pos2.getX() || oldPos.getX() - size1.getX() > pos2.getX() + (size2.getX() * size2.getX()) || newPos.getY() + size1.getY() < pos2.getY() || newPos.getY() - size1.getY() > pos2.getY() + (size2.getY() * size2.getY())))
			result.setY(0);
...
//		nearestIntersect = collision;
	...
	public static Vector2f lineIntersectRect(Vector2f lineStart, Vector2f lineEnd, Vector2f rectStart, Vector2f rectSize) {
		...
		if (collision != null && (result == null || result.sub(lineStart).length() > collision.sub(lineStart).length()))
			result = collision;
